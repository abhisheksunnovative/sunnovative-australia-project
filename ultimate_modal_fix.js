...
function patchFile(filepath) {
    ...
    // Clean up ALL previous empty tags
    text = text.replace(/\{\/\* Customer Details Modal \*\/\}\s*/g, '');
    
    const targetStr1 = '    </div>\r\n  );\r\n}';
    const targetStr2 = '    </div>\n  );\n}';
    
    let targetStr = text.includes(targetStr1) ? targetStr1 : targetStr2;
    
    const idx = text.lastIndexOf(targetStr);
...
function cleanAndPatch(filepath) {
    let text = fs.readFileSync(filepath, 'utf-8');
    text = text.replace(/\{\/\* Customer Details Modal \*\/\}\s*/g, '');
    let arr = text.split(/<\/\s*div\s*>\s*<\/\s*div\s*>\s*;\s*\}\s*$/);
    
    // just use Regex replace at the end of the file.
    let cleanText = text.replace(/(<\/\s*div\s*>\s*)\);\s*\}\s*$/, '$1);\n}');
    // find the end
    let match = cleanText.match(/(<\/\s*div\s*>\s*)\);\s*\}\s*$/);
    if(match) {
        cleanText = cleanText.replace(/(<\/\s*div\s*>\s*)\);\s*\}\s*$/, '      {/* Customer Details Modal */}\n      ' + modal + '\n$1);\n}\n');
        fs.writeFileSync(filepath, cleanText);
        console.log("Replaced with regex for " + filepath);
    } else {
        console.log("Regex replace failed for " + filepath);
    }
}

cleanAndPatch('Website_Admin/src/components/bde/BDEProspects.jsx');
cleanAndPatch('Website_Admin/src/components/bde/BDELeadManagement.jsx');
