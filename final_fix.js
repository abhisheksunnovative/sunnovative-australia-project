...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('if (!settings) {')) {
    startIdx = i;
  }
  if (lines[i].includes('const rules = settings.eligibilityRules;')) {
    endIdx = i;
    break;
  }
}
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  lines.splice(startIdx, endIdx - startIdx + 1, newSeeder);
  fs.writeFileSync(path, lines.join('\\n'));
  console.log("Replaced successfully!");
} else {
  console.log("Could not find boundary lines");
}
