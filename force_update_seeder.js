...
const startIdx = text.indexOf('if (!settings) {');
const endIdx = text.indexOf('// 1. First derive from Monthly Units');

if (startIdx !== -1 && endIdx !== -1) {
  const newSeeder = `if (!settings) {
  ...
  fs.writeFileSync(path, before + newSeeder + after);
  console.log("Seeder updated properly!");
} else {
  console.log("Could not find seeder boundaries");
}
