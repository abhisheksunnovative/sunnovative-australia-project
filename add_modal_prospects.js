...
// First remove the empty "{/* Customer Details Modal */}" if it exists
text = text.replace('{/* Customer Details Modal */}\\n      \\n    </div>\\n  );\\n}\\n\\n', '    </div>\\n  );\\n}\\n');
text = text.replace('{/* Customer Details Modal */}\\n      \\n    </div>\\n  );\\n}', '    </div>\\n  );\\n}');

const idx = text.lastIndexOf('    </div>\\n  );\\n}');
if (idx !== -1) {
    const newText = text.substring(0, idx) + '      {/* Customer Details Modal */}\\n      ' + modal + '\\n    </div>\\n  );\\n}';
    fs.writeFileSync('Website_Admin/src/components/bde/BDEProspects.jsx', newText);
    console.log("Replaced BDEProspects");
} else {
    console.log("Could not find insertion point in BDEProspects");
}
