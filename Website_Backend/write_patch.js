...
const file = path.join('d:', 'sunnovative-australia-website', 'Website_Backend', 'src', 'controllers', 'lightBillScanController.js');

let code = fs.readFileSync(file, 'utf8');

// The block to replace starts at `// Fallback removed` and ends right before `// Tariff Category Verification`
...
const replacement = `// Fallback removed as per user request (no guess-multiply for low kWh)
        ...
        // If critical fields are missing OR confidence < 70, flag for Template Builder review
            ...
            if (isAU) {
                ...
                if (initialConfidence < 70) {
                    ...
                    if (geminiData) {
                        ...
                        // If Gemini fixed recency, we'd theoretically check it again, but usually it fixes amounts.
                    } else {
                        engineUsed = 'in-house-failed';
                        fallbackReason = fallbackReason || 'Gemini Fallback failed or unavailable';
                    }
                } else {
                    // India bill >= 70% confidence, but critical fields missing (e.g. date too old)
                    // Do NOT use Gemini. Send to Template Builder.
                    engineUsed = 'in-house-failed';
        ...
        try {
            await ScanAnalytics.create({
                ...
                engineUsed,
                geminiCost,
                needsTemplateCreation: needsTemplate, // Will populate Admin Dashboard Queue
                rawText: rawText,
...
// Note: I also replaced the ScanAnalytics block above because it makes sense to log `initialConfidence` or we could log the final confidence. But I'll replace everything up to "// Tariff Category Verification". 
// Wait, I should find exactly where the ScanAnalytics block ends. Let's look at the original code.
`;

fs.writeFileSync('d:/sunnovative-australia-website/Website_Backend/patch_unified_scan.cjs', 
`
const fs = require('fs');
...
// Also fix the final confidenceScore calculation at the bottom of the function.
...
console.log("Unified India and AU logic with 70% threshold for Gemini.");
`);
