...
const regexGeneratorFunc = `// 5. Generate Regex from Highlighted Text (100% Offline, Pure Math v3)
export const generateRegexFromSelection = async (req, res) => {
  try {
    ...
    // Escape regex characters using a trick to avoid $& replace bugs in node scripts
    ...
    if (anchor) {
      ...
      regexStr = "(?:" + anchor + ")[\\\\s\\\\n:]{0,20}?" + captureGroup;
       ...
       // If no anchor, match exact text replacing digits with \\\\d+
       let escapedText = escapeRegex(selectedText);
       regexStr = escapedText.replace(/\\d+/g, '\\\\d+');
    }
...
fs.writeFileSync(path, code + regexGeneratorFunc);
console.log("Fixed $& bug by using simple char loop escape! File appended properly!");
