...
const regexAmount = /const amountPatterns = \[[\s\S]*?\];\s*for \(const p of amountPatterns\) \{\s*const m = t\.match\(p\);\s*if \(m\) \{\s*quarterlyBillAmount = parseFloat\(m\[1\]\.replace\(\/,\/g, ''\)\);\s*break;\s*\}\s*\}/m;
...
if (regexAmount.test(content)) {
  content = content.replace(regexAmount, newAmount);
  fs.writeFileSync('src/utils/Ocrextractor.js', content);
  console.log("Amount priority patched!");
} else {
  console.log("Amount regex not found!");
}
