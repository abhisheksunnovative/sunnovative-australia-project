...
if (auTemplateRegex.test(btc)) {
  btc = btc.replace(auTemplateRegex, newAuTemplate);
  fs.writeFileSync(btcPath, btc);
  console.log('billTemplateController actually updated this time!');
} else {
  console.log('Could not find auTemplateRegex in billTemplateController.js');
}
