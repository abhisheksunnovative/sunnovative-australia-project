...
const templates = [
  {
    ...
    extractionRules: [
      ...
      { field: "tariffCategory", regex: "Tariff Category[\\s\\S]{0,30}?\\s*([A-Za-z \\[\\]]+)", type: "string", required: false },
      { field: "meterTypeInfo", regex: "Meter Type[\\s\\S]{0,20}?\\s*([A-Za-z0-9]+)", type: "string", required: false },
      { field: "postcode", regex: "\\b([0-9]{6})\\b", type: "string", required: false }
    ]
  },
  {
    discomName: "PSPCL",
    ...
    extractionRules: [
      ...
      { field: "tariffCategory", regex: "(?:Billing Category|Category)[\\s\\S]{0,30}?([A-Z0-9\\-]+)", type: "string", required: false },
      { field: "billingDays", regex: "Bill Period[\\s\\S]{0,20}?([0-9]{2,3})", type: "number", required: false },
      { field: "postcode", regex: "\\b([0-9]{6})\\b", type: "string", required: false }
    ]
  },
  {
    ...
    extractionRules: [
      ...
      { field: "tariffCategory", regex: "Category[\\s\\S]{0,20}?([A-Za-z0-9\\-]+)", type: "string", required: false },
      { field: "postcode", regex: "\\b([0-9]{6})\\b", type: "string", required: false }
    ]
  },
  {
    ...
    extractionRules: [
      ...
      { field: "tariffCategory", regex: "([A-Za-z]+ [A-Za-z]+ plan)", type: "string", required: false },
      { field: "meterTypeInfo", regex: "Meter[\\s\\S]{0,10}?([0-9]{5,})", type: "string", required: false },
  ...
  },
  {
    ...
    extractionRules: [
      ...
      { field: "meterTypeInfo", regex: "meter number:\\s*([0-9]+)", type: "string", required: false },
      { field: "state", regex: "\\b(NSW|VIC|QLD|WA|SA|TAS|ACT|NT)\\b", type: "string", required: false },
      { field: "postcode", regex: "\\b([0-9]{4})\\b", type: "string", required: false }
    ]
  }
...
async function seedTemplates() {
  try {
    ...
    for (let t of templates) {
      ...
      console.log(`Upserted full template for: ${t.discomName}`);
    }

    console.log("Super comprehensive OCR Seeding complete!");
    process.exit(0);
  } catch (err) {
