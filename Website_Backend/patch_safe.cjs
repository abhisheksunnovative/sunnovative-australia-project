...
const replacement1 = `        // Fallback removed as per user request (no guess-multiply for low kWh)
...
const replacement2 = `        // Final Combined Confidence Scoring (After potential Gemini merge)
        ...
        if (confidenceScore < 50) status = 'manual-review';`.split('\n');
