...
const tariffRegex = /let tariffType = null;\s*if \(\/Time\\s\*of\\s\*Use\|TOU\|Peak\.\*Shoulder\|Shoulder\.\*Peak\/is\.test\(t\)\) tariffType = 'Time of Use \(TOU\)';\s*else if \(\/Single\\s\*Rate\|Flat\\s\*Rate\/i\.test\(t\)\) tariffType = 'Single Rate';\s*else if \(\/Controlled\\s\*Load\|Off\.\?Peak\/i\.test\(t\)\) tariffType = 'Controlled Load';/m;
...
if (tariffRegex.test(content)) {
  content = content.replace(tariffRegex, tariffNew);
  fs.writeFileSync('src/utils/Ocrextractor.js', content);
  console.log("Patched Tariff extraction with Regex");
} else {
  console.log("Tariff regex not matched!");
}
