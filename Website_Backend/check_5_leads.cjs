...
mongoose.connect(MONGODB_URL).then(async () => {
  ...
  const leads = await Lead.find({ assignedBde: bdeId, name: { $in: ['James Wilson', 'Sarah Thompson', 'Michael Chen', 'Emma Rodriguez', 'David Nguyen'] } });
  console.log("Newly assigned leads:", leads.length);
  if (leads.length > 0) {
    console.log("History of first:", leads[0].history);
    console.log("Status of first:", leads[0].status);
  }
