...
export const checkBillEligibility = async (req, res) => {
  ...
  try {
    ...
    if (monthlyUnits && kdr.unitsPerKW) {
      ...
      // Enforce minimum of 1 kW (or configured min)
      const minKw = rules.minKW || 1;
      unitsDerivedKw = Math.max(minKw, unitsDerivedKw);
      
      const billDerivedUnits = billAmount ? (billAmount / 7.2) : 0;
      const billDerivedRawKw = billDerivedUnits / kdr.unitsPerKW;
      
      if (billAmount && billDerivedRawKw > (raw * 2.5)) {
        console.log('[Eligibility] OCR units wildly low compared to bill.');
        unitsDerivedKw = null;
      } else {
