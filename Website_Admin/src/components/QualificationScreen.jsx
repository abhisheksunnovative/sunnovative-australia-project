...
export const QualificationScreen = () => {
  ...
  const fetchQualifications = async (country, state) => {
    try {
      setLoading(true);
      const res = await fetch(`/api/qualifications?country=${country}&state=${state}`);
  ...
  const handleSaveRule = async () => {
    ...
    try {
      ...
      const url = editingRule ? `/api/qualifications/${editingRule._id}` : '/api/qualifications';
      ...
      {level === "rules" && (
        <div className="space-y-6">
          ...
          {loading ? (
            ...
            <EmptyState icon={<Award />} title="No Categories Found" description={`No EPC Qualification categories set up for ${selectedState} yet.`} />
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {qualifications.map(rule => (
                <div key={rule._id} className={`bg-white rounded-2xl border ${rule.isActive ? 'border-emerald-200 shadow-sm' : 'border-slate-200 opacity-70'} overflow-hidden`}>
                  <div className={`p-4 border-b ${rule.isActive ? 'bg-emerald-50/50 border-emerald-100' : 'bg-slate-50 border-slate-100'} flex justify-between items-center`}>
