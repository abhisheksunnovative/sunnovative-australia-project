...
export default function TrustBadgeEpcScreen ({
  ...
  const { projectTypes: dynamicProjectTypes } = useAdminSettings(cardCountry || '');
  const dynamicProjectTypeList = dynamicProjectTypes.length > 0 ? dynamicProjectTypes.map(pt => pt.value) : ["Residential", "Commercial", "Group Solar", "Village Solar Campaign"];
    ...
    try {
      ...
      if (response.ok) {
        ...
        const mappedData = rawList.map(dbP => ({
          ...
          projectTypes: (dbP.qualifiedProjectTypes && dbP.qualifiedProjectTypes.length > 0) ? dbP.qualifiedProjectTypes : ["Residential"],
        ...
        setDbPartners(mappedData.filter(e => e.trustBadge?.status && e.trustBadge.status !== "None"));
        if (paginationInfo) {
      ...
      console.error("Error fetching partners:", err); setDbPartners([{ id: "err", partnerName: "ERROR: " + err.message, companyName: "ERROR", country: "india", trustBadge: { status: "Pending" } }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Fetch country list for card filters â€” use COUNTRY_DATA directly (no API needed)
  useEffect(() => {
  ...
  const filteredPartners = dbPartners.filter((p) => {
    ...
    const matchesCardProjectType = !cardProjectType || (p.projectTypes || []).some(t => cardProjectType.toLowerCase().includes(t.toLowerCase()) || t.toLowerCase().includes(cardProjectType.toLowerCase()));
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {dynamicProjectTypeList.map(pt => {
                  const count = dbPartners.filter(p =>
                    ...
                    (p.projectTypes || []).some(t => pt.toLowerCase().includes(t.toLowerCase()) || t.toLowerCase().includes(pt.toLowerCase()))
                  ).length;
                  return (
                    <div 
                      key={pt} 
...
{/* --- DATA TABLE: RENDER PRIMARY ACTIVE EPC GRID --- */}
      {!currentFormOpen && cardCountry && cardState && cardProjectType && filteredPartners.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-xs overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-3.5 border-t border-gray-100 bg-gray-50/50">
              ...
              <p className="text-xs text-gray-500">
                ...
                <span className="font-semibold text-primary">
                  {(currentPage - 1) * ITEMS_PER_PAGE + 1}â€“{Math.min(currentPage * ITEMS_PER_PAGE, totalCount)}
                </span>{" "}
              ...
              <div className="flex items-center gap-1">
                ...
                {(() => {
                  ...
                  return pages.map((pg, idx) =>
                    pg === '...' ? (
                      <span key={`ellipsis-${idx}`} className="px-2 text-xs text-gray-400">â€¦</span>
                    ) : (
                      <button
                        key={pg}
          ...
          {/* When only 1 page â€” show simple count */}
              ...
              {profileActiveTab === "trustBadge" && (
                <div className="space-y-4">
                  ...
                  <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border ${
                    ...
                    â— Status: {selectedPartner.trustBadge?.status || 'None'}
                  </div>
                      ...
                      {selectedPartner.trustBadge.applicationDetails.paymentSlipUrl && (
                        <div>
                          ...
                          <a href={selectedPartner.trustBadge.applicationDetails.paymentSlipUrl} target="_blank" rel="noreferrer">
                            ...
                            />
                            <p className="text-[10px] text-blue-500 mt-1 text-center">Click to open full size â†—</p>
                          </a>
                        </div>
                      )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                    ...
                    <div className="p-3 border rounded-xl">
                      ...
                      <p className="font-bold mt-1 text-primary">
                        {selectedPartner.plan === "Enterprise Plan"
                        ...
                        >
                          <div>
                            ...
                            <span className="text-[10px] text-gray-400 block mt-0.5">
                              {proj.kwSize}kW Solar Â· {proj.location},{" "}
                              {proj.state}
                            </span>
                          </div>
                        ...
                        >
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400 mt-0.5">
                              {inst.experienceYears} Years experience Â·{" "}
                              {inst.mobile}
                            </p>
                          </div>
