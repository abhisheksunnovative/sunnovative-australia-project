...
export const KycScreen = () => {
  ...
  // Hardcoded for now, ideally fetched from a Settings API
  const liveCountries = [
    { name: "India", code: "IN", icon: "🇮🇳" },
    { name: "Australia", code: "AU", icon: "🇦🇺" }
  ];

  // Dummy states based on country
  ...
  const fetchEpcs = async (country, state) => {
    try {
      ...
      const res = await fetch(/api/kyc/epcs?country=&state=, {
  ...
  const fetchRequirements = async (country, state) => {
    try {
      const token = localStorage.getItem("token") || "admin_super_secret_token_12345";
      const res = await fetch(/api/kyc/requirements?country=&state=, {
        headers: { Authorization: Bearer  }
      });
      const data = await res.json();
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      const token = localStorage.getItem("token") || "admin_super_secret_token_12345";
      await fetch(/api/kyc/epcs//status, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: Bearer  },
      ...
      {level === "epcs" && (
        <div className="space-y-6">
          ...
          {loading ? (
            ...
            <EmptyState icon={<ShieldCheck />} title="No EPCs Found" description={No EPCs registered in  yet.} />
          ) : (
...
print("KycScreen replaced with Dynamic Drill-down UI")
