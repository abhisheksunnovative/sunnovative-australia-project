...
const inRegex = /<div className=\{`border-2 border-dashed rounded-xl p-3 text-center transition[\s\S]*?View Document\s*<\/a>\s*\}\s*<\/div>\s*\)\s*:\s*\([\s\S]*?Tap to upload terrace photo \/ light bill\s*<\/p>\s*\)\s*\}\s*<\/div>/;
