...
const oldContainer = `<div className="w-20 h-20 rounded-2xl bg-solar-navy shadow-lg border border-slate-800 flex items-center justify-center mb-6 transition-transform duration-300 group-hover:-translate-y-1 relative">
                {item.icon}
...
const newContainer = `<div className="w-24 h-24 rounded-3xl bg-white shadow-lg border border-slate-100 flex items-center justify-center mb-6 transition-transform duration-300 group-hover:scale-110 group-hover:-translate-y-1 relative">
                {item.icon}
...
if(content.includes(oldIconLogic)) {
    ...
    console.log("Successfully reverted OrderJourneySteps to use realistic 3D images.");
} else {
