...
const inRegex = /<div className=\{`border-2 border-dashed rounded-xl p-3 text-center transition[\s\S]*?View Document<\/a>\s*\)\s*\}\s*<\/div>\s*\)\s*:\s*\([\s\S]*?Tap to upload terrace photo \/ light bill\s*<\/p>\s*\)\s*\}\s*<\/div>/;
...
if (code.match(inRegex)) {
  ...
  console.log("Patched IN UI again!");
